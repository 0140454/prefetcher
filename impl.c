...
void sse_prefetch_transpose(int *src, int *dst, int w, int h)
{
    for (int x = 0; x < w; x += 4) {
        for (int y = 0; y < h; y += 4) {
#define SSE_PFDIST  8
            _mm_prefetch(src+(y + SSE_PFDIST + 0) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + SSE_PFDIST + 1) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + SSE_PFDIST + 2) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + SSE_PFDIST + 3) *w + x, _MM_HINT_T1);
